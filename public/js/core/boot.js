...
function NXbootstrap($self, action) {
    ...
    $(document).ready(function() {
        ...
        // });
        $(".select2-basic-with-search").each(function () {
            var placeholder = $(this).data('placeholder') ? $(this).data('placeholder') : '';
 ...
 *     - uses lang as sent in header template
 * -----------------------------------------------------------*/
$(document).on('click', '.confirm-action-red, .confirm-action-danger, .confirm-action-info, .confirm-action-primary, .confirm-action-success', function() {
    //the clicked element
    var $self = $(this);
    ...
    //error - red theme
    var confirm_popup_button_class = ' btn-sm btn-outline-red';
    var confirm_popup_theme_style = 'danger';

    //info - blue theme
    if ($self.hasClass('confirm-action-danger')) {
        //default - green theme
    ...
    //info - blue theme
    if ($self.hasClass('confirm-action-primary')) {
        var confirm_popup_button_class = ' btn-sm btn-primary';
        var confirm_popup_theme_style = 'primary';
    }
    ...
    $.confirm({
        ...
        buttons: {
            no: {
                text: NXLANG.cancel,
                btnClass: 'btn-sm btn-secondry',
            },
            yes: {
                text: $self.attr('data-confirm-button') ?? NXLANG.continue,
 ...
 *   select users to show in assigned drop downn list
 * -----------------------------------------------------------*/
NX.projectManagersCToggle = function(e, $self) {

    //the company id
    var company_id = e.params.data.id;

    //the projects dropdown list
    var assigned_dropdown = $("#" + $self.attr('data-managers-dropdown'));
    ...
    $.ajax({
        type: 'GET',
        url: NX.site_url + "/feed/managers?company_id=" + company_id
    }).then(function(data) {
 ...
 * -----------------------------------------------------------*/
NX.eventsTopNavMarkAsRead = function($self) {
    //request
    nxAjaxUxRequest($self);
}
...
NX.browserPushState = function($self) {
    ...
    if (typeof new_url != 'undefined' && new_url != null) {
        window.history.pushState(null, null, NX.site_url + '/' + new_url);
    }
 ...
 * -------------------------------------------------------------------------------------*/
NX.NXInventroyMultiItem = function(e) {

    $('#inventory-items .item').each(function(i) {
        //var i = i+1;
        $(this).find('.note-item-qty').text((i+1));
        $(this).find('input[name^="item_id"]').attr('name', 'item_id[' + i + ']');
        $(this).find('input[name^="item_code"]').attr('name', 'item_code[' + i + ']');
        $(this).find('input[name^="quantity"]').attr('name', 'quantity[' + i + ']');
        $(this).find('input[name^="unit_price"]').attr('name', 'unit_price[' + i + ']');
        $(this).find('input[name^="total_price"]').attr('name', 'total_price[' + i + ']');
    });

    // $('.inventory-items .item').each(function(i) {
    //     $(this).find('select[name^="item"]').attr('name', 'item[' + i + ']');
    //     $(this).find('input[name^="item_quantity"]').attr('name', 'item_quantity[' + i + ']');
    //     $(this).find('input[name^="item_note"]').attr('name', 'item_note[' + i + ']');
    //     $(this).find('input[name^="item_packed"]').attr('name', 'item_packed[' + i + ']');
    //     $(this).find('input[name^="item_packed"]').attr('id', 'item_packed[' + i + ']');
    //     $(this).find('input[name^="item_packed"]').next().attr('for', 'item_packed[' + i + ']');
    // });
}

/**--------------------------------------------------------------------------------------
 * SERVICE ORDER - INVENTORY ITEMS] 
 * @description: edit projects inventory
 * -------------------------------------------------------------------------------------*/
NX.NXServiceOrdersMultiItem = function(e) {

    $('#service-order-items .item').each(function(i) {
        //var i = i+1;
        $(this).find('.item-count').text((i+1));

        $(this).find('input[name^="item_id"]').attr('name', 'item_id[' + i + ']');
        $(this).find('input[name^="item_type"]').attr('name', 'item_type[' + i + ']');
        $(this).find('select[name^="selected_item"]').attr('name', 'selected_item[' + i + ']');
        $(this).find('input[name^="selected_item"]').attr('name', 'selected_item[' + i + ']');
        $(this).find('input[name^="item_qty"]').attr('name', 'item_qty[' + i + ']');
        $(this).find('input[name^="item_remark"]').attr('name', 'item_remark[' + i + ']');
        $(this).find('input[name^="item_mileage"]').attr('name', 'item_mileage[' + i + ']');
        $(this).find('input[name^="item_price"]').attr('name', 'item_price[' + i + ']');
    });
}

/**--------------------------------------------------------------------------------------
 * SIDEBAR MENU - TABS MEMU TOGGLE] 
 * @description: simple show/hide content on click tab menu sidebar
 * -------------------------------------------------------------------------------------*/
$(document).ready(function() {
    $(document).on('click', '.side-nav-tabs > li > a', function(event) {
        event.preventDefault(); //stop browser to take action for clicked anchor

        //get displaying tab content jQuery selector
        var active_tab_selector = $('.side-nav-tabs > li.active > a').attr('href');

        //find actived navigation and remove 'active' css
        var actived_nav = $('.side-nav-tabs > li.active');
...
$(document).ajaxComplete(function() {
    ...
    if (selectedTab) {
        ...
        var tab_selector = $('.side-nav-tabs > li > a[href="' + selectedTab + '"]');
...
$(document).ajaxComplete(function() {
    ...
    if ($("#attendances-table-wrapper").length) {
        /** ---------------------------------------------------
        ...
        $(document).off('click', '#card-attendance-update-remark-button').on('click', '#card-attendance-update-remark-button', function() {
 ...
 *---------------------------------------*/
//remove an element
$(document).on('change', '.item-packed-check', function(e) {
    //send request
    nxAjaxUxRequest($(this));
});
...
/*----------------------------------------------------------------
    *  [input data save]
    * ------------------------------------------------------------*/
$(document).on("focusout", '.remark-input', function() {
    nxAjaxUxRequest($(this));
});
$(document).on("change", '.switch .js-ajax', function() {
    nxAjaxUxRequest($(this));
});