			...
			$.each(payload, function (index, value) {
				...
				if (typeof value == 'object') {
					if (value.selector != undefined && value.action != undefined) {
						...
						//disable
						if (value.action == 'form-reset') {
							$(value.selector).trigger("reset");
						}//disable
						if (value.action == 'dropzone-reset') {
								Dropzone.forElement("#fileupload_user_document").removeAllFiles(true);
						}
					}
				}
			});
		...
		error: function (data, jqXHR) {
			...
			if (typeof data.responseJSON != 'undefined') {
					...
					if (data.status == 409) {
						...
						var notify = JSON.parse(data.responseJSON.message);
						//verify if data is type notification
						if(Object.keys(notify.notification).length){
							// payload
							NXAJAX.getPayload(notify);
						}
					}else{
						//payload
