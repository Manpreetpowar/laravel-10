        ...
        <div class="card-footer bg-white text-center border-0">
            @if($machine->operator && $machine->operator->id != auth()->user()->id)
                <button type="button" title="Operate Machine"
                        class="data-toggle-action-tooltip btn btn-primary btn-circle confirm-action-info mb-2"
            ...
            @elseif($machine->operator && $machine->operator->id == auth()->user()->id)
                <a href="{{url('floor-operations/machines/'.$machine->id.'/operate')}}" class="btn btn-primary mb-2">
                    Operate Machine
                </a>
                <button type="button" title="Leave Machine"
                    class="data-toggle-action-tooltip btn btn-outline-danger btn-circle confirm-action-danger mb-2"
