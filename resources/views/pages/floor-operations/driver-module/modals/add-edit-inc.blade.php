@csrf
  <div class="form-row">
    <div class="form-group col-md-6">
      ...
      <select id="client" name="client" class="form-control select2-basic-with-search" data-placeholder="Select Client" tabindex="-1" aria-hidden="true">
            <option value="" disabled selected>Select Customer</option>
            @foreach($clients as $client)
            <option value="{{$client->id}}" {{ isset($client) ? runtimePreselected($client->id, 0) : ''}}>{{$client->client_name}}</option>
            @endforeach
      </select>
    </div>
   ...
   @if(auth()->user()->hasRole('administrator'))
        <div class="form-group col-md-6">
            ...
            <select id="driver" name="driver" class="form-control select2-basic-with-search" data-placeholder="Select Driver" tabindex="-1" aria-hidden="true">
                    <option value="" disabled selected>Select Driver</option>
                    @foreach($drivers as $driver)
                    <option value="{{$driver->id}}" {{ isset($driver) ? runtimePreselected($driver->id, 0) : ''}}>{{$driver->name}}</option>
                    @endforeach
            </select>
        </div>
    @endif 
